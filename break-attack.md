...
사생활 문제는 공격자 개념이 포함되어 있지 않습니다. 이 분야는 learning algorithm이 트레이닝 데이터를 가지고 학습할 때 공평성과 투명성이 부족한 점을 이용합니다. 최근 연구를 보면 트레이닝 데이터 셋이 사회적으로 편향되어 있으면 모델도 편향된다는 점입니다. 하지만, 우리는 공격자가 있는 시나리오를 볼겁니다.
