...
공격자는 트레이닝 된 모델에서 정보를 복구하거나, 중요한 정보를 뽑아내려고 합니다. 스마트폰을 보면, 사용자가 입력한 키보드 패턴을 저장하고 예측하여 자동완성에 도움을 줍니다. 그러나, 사용자의 특정한 입력은 다른 핸드폰에 나타나지 않아야 합니다. 사생활 공격은 inference 단계에서 이루어집니다. 이러한 문제를 해결하려면 learning algorithm에 randomization이 들어가야합니다.

공격자가 회원에 대한 정보를 얻으려고 시도한다고 해봅시다... 